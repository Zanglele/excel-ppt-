...
def run_check(output_directory: str) -> int:
    ...
    try:
        ...
        try:
            ...
            sheet.append(["模拟客户甲", "XRF", 98, 1234])
            sheet.append(["模拟客户乙", "BFI", 97, 567])
            sheet.append(["模拟未保客户", "XRF", 96, 89])
            ...
            sheet.append(["XRF", "2026-1-2", "001", "申请关闭"])
            sheet.append(["XRF", "2026-1-3", "001", "已结束"])
            sheet.append(["XRF", "2026-2-4", "002", "处理中"])
            sheet.append(["BFI", "2026-1-2", "003", "已取消"])
            book.save(second)
            book.close()
